...
// 나의 트래킹 이미지 삭제
router.delete("/:tracking_idx", checkTrackingIdxData(), deleteTrackingImg);

// 트래킹 라인 가져오기
router.get("/:tracking_idx", checkTrackingIdxData(), getTrackingLine);

// 트래킹 이미지 수정
router.put("/:tracking_idx", checkTrackingIdxData(), putTrackingImage);

// 트래킹 이미지 공유 상태 변경
router.put("/", checkSetData("idxList"));
